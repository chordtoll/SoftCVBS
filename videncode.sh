...
make ntsc-decode
make deinterlace
ffmpeg -i input/$1 -c:v rawvideo -f rawvideo -vf scale=640:480 -pix_fmt rgb24 - | ./ntsc-encode | ./ntsc-decode | ./deinterlace | ffmpeg -y -f rawvideo -pix_fmt rgb24 -s:v 800x800 -r 30 -i - -filter:v "crop=647:480:19:26" tempencode/vidcvbs.mp4
ffmpeg -y -i input/$1 tempencode/vid.mp3
...
/mnt/c/Program\ Files\ \(x86\)/Audacity/audacity.exe c:\\users\\asent\\desktop\\python\\ntsc\\sdr-examples\\ntsc\\tempencode\\vidcompress.wav
read -p "Press enter to continue"
ffmpeg -y -i tempencode/vidcvbs.mp4 -i tempencode/vidcompress.mp3 -c:v copy -c:a copy output/$1